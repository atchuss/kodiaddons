...
free = 'aHR0cDovL2xlZ2lvbndvcmxkdHYuY29tL2xpc3QvbWFpbi50eHQ='.decode('base64')
...
SOURCES = [{"title": "Golde Cowboys ", "url": free, "fanart": "https://img.fotor.com/share/1471478420006KbP.jpg", "genre": "IPTV", "date": "07.11.2016", "credits": "FREEWORLD", "thumbnail": "https://img.fotor.com/share/1471478505364tQW.jpg"}]

#import urllib
#f = urllib.urlopen('http://legionworldtv.com/list/seriecatalogo.txt')
#paggg = f.read()
#xbmc.log("JJ==========================================JJ")
#xbmc.log("GOLDEN_JJSR_Sources: %s" %(str(paggg)))
#xbmc.log("JJ==========================================JJ")
...
def getSoup(url,data=None):
        ...
        if url.startswith('http://') or url.startswith('https://'):
            ...
            if enckey:
                    ...
                    xbmc.log("GOLDEN_JJSR_Data64: %s" %(str(data)))
...
#       print 's is ',s
        ...
        xbmc.log("GOLDEN_JJSR_s64: %s" %(str(s)))
        s= urllib.unquote(s);
    print s
    return s
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
	...
	dik = {'170':'ª','201':'É','209':'Ñ','211':'Ó','205':'Í','243':'ó','193':'Á','237':'í','191':'¿','225':'á'}
        name = name.replace('[COLOR blue][/COLOR]','[COLOR gold]') #333
        name = name.replace('&amp;','&')
        xbmc.log("BLUE_JJSR_Name: %s" %(str(name)))
        #name = name.replace('[COLOR blue]  •[/COLOR][COLOR orange]','[COLOR gold]')
        name = re.sub('(\[.*ge\])', '[COLOR gold]', name)
        malo = re.findall('&#(.+?);',name)
        for x in (malo):
            try:
                otr = dik[x]
            except:
                otr = '@' + str(x)
            name = name.replace('&#' + x + ';', otr)

	lta = ('^(.*?)[\]\)\[\(]','[\]\)](.+?)[\[\(]','.*[\]\)\[\(](.*?)$')
        hay = False
        for gg in lta:
            malo = re.findall(gg,name)
            for x in (malo):
                hay = True
        ...
        name = re.sub('(\.(avi|mkv|mpe?g|mp4|flv)?)', '', name, flags=re.IGNORECASE)

        malo = re.findall('.*?\d*?(X)\d+',name)
        for x in (malo):
...
if not url is None:
    ...
    xbmc.log("GOLDEN_JJSR_Name: %s" %(str(name)))
    xbmc.log("GOLDEN_JJSR_Url: %s" %(str(url.encode('utf-8'))))
...
if not playitem =='':
    ...
    xbmc.log("GOLDEN_JJSR_PlName: %s" %(str(name)))
    xbmc.log("GOLDEN_JJSR_PlUrl: %s" %(str(url.encode('utf-8'))))
