...
def mainlist(item):
    ...
    itemlist = []
    itemlist.append( Item(channel=item.channel, title="[COLOR skyblue][B]Géneros[/B][/COLOR]"      , action="generos", url="http://bityouth.com", thumbnail="http://s6.postimg.org/ybey4gxu9/bityougenerosthum3.png", fanart="http://s18.postimg.org/l4judlx09/bityougenerosfan.jpg"))
    itemlist.append( Item(channel=item.channel, title="[COLOR skyblue][B]Puntuación[/B][/COLOR]"      , action="scraper", url="http://bityouth.com/more_elements/0/?o=pd", thumbnail="http://s6.postimg.org/n1qtn9i6p/bityoupuntothum4.png", fanart="http://s6.postimg.org/qrh9oof9t/bityoupuntofan.jpg"))
...
def search(item,texto):
    ...
    for scrapedrate, scrapedurl, scrapedtitle, scrapedthumbnail, scrapedyear, scrapedplot  in matches:
        ...
        if " /10" in scrapedrate:
            scrapedrate= scrapedrate.replace(" /10"," ---")
        ...
        if not "serie_de_tv" in item.url:
            scrapedtitle = scrapedtitle.replace("(Serie de TV)","[COLOR darkorange](Serie de TV)[/COLOR]")
        else:
...
def findvideos_pelis(item):
    ...
    for scrapedurl, scrapedcalidad, scrapedsize in matches:
        ...
        itemlist.append( Item(channel=item.channel, title=title, fulltitle=fulltitle, url=scrapedurl, fanart=item.show, thumbnail=item.extra, action="play", folder=False) )
...
def play(item):
    ...
    xbmc.log("WW_JJSR_channel..: %s" %(item.fulltitle))
    #item.title = item.oldtitle
    #item.plot = item.oldtitle
    ...
    for scrapedurl in matches:
        ...
        itemlist.append( Item(channel=item.channel, title=item.title, fulltitle=item.fulltitle, server="torrent", url=scrapedurl, fanart="http://s9.postimg.org/lmwhrdl7z/aquitfanart.jpg", thumbnail=item.thumbnail, action="play", folder=True) )
